...
const Concept = ({ isCard, name, imgSrc, info }) => {
  ...
  if (isCard) {
    return (
      <div className="relative flex flex-col items-center w-64 h-80 bg-white rounded-3xl shadow-lg border-2 border-gold overflow-hidden hover:scale-105 transition-transform duration-300 cursor-pointer group focus-within:ring-4 focus-within:ring-gold" tabIndex={0} aria-label={name[lang]}>
        <img
          src={imgSrc}
          alt={name[lang]}
        ...
        <div className="flex-1 flex flex-col justify-center items-center p-4 w-full">
          ...
          <p className="text-background text-center text-sm line-clamp-3 group-hover:line-clamp-none group-hover:text-gold transition-all duration-300">
            {info && info[0] && info[0][lang] && info[0][lang].title}
          </p>
        </div>
